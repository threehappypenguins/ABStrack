...
/**
 * Form to create an empty health marker preset header, then navigate to the editor for lines.
 *
 * @returns Create preset page content.
 */
export function HealthMarkerPresetCreateForm() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    announce('Preset created. Add markers below.', { politeness: 'polite' });
    router.push(`/presets/health-markers/${result.data.id}`);
  };
    ...
    <div className="w-full space-y-8">
      <div>
        ...
        <h1 className="mt-4 text-2xl font-bold tracking-tight text-app-ink">
          New health marker preset
        </h1>
        <p className="mt-1 text-sm text-app-muted">
          Choose a name, then add markers (BAC, glucose, blood pressure, and
      ...
      >
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          ...
          >
            {saving ? 'Creating…' : 'Create and edit markers'}
          </button>
          <Link
            href="/presets/health-markers"
