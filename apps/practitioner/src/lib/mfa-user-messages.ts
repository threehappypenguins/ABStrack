...
export function getVerificationMessage(message?: string): string {
...
export type AuthLikeErrorFields = {
  message: string;
  status?: number;
  code?: string;
};
 ...
 */
export function readAuthLikeError(error: unknown): AuthLikeErrorFields {
 ...
 * @returns Message for `setAccessibleStatus` / announcements.
 */
