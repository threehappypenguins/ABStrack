...
- `AppNotFoundPage` — panel with optional `<main id="main-content">` (`wrapInMain`, default `true`) and optional `topNav`. Set `wrapInMain={false}` when a route-group layout already provides `<main>`.
