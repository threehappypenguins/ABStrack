...
| Merge into **`dev`**               | Real deploy when paths change: Supabase Cloud `db push`, PowerSync Cloud `deploy sync-config`.        |
| PR **`dev` → `main`**              | Automatic lightweight checks (fine to run without a comment).                                         |
| Merge into **`main`**              | Real prod deploy when paths change: self-hosted Supabase + PowerSync via Tailscale.                   |

#### PR comment commands

Post the comment as the **entire** PR comment body (exact match; surrounding whitespace only). You need **write** access on the repo. Workflows react to the comment and post a **commit status** on the PR head SHA (for branch protection).

| Comment         | When to use                                                                                                | What runs                                                                                                                                                                                                                                                                                                                                                                                                                         | Commit status context(s)                                                           |
| --------------- | ---------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| **`/ci`**       | Before merging app/code changes into `dev` (any PR); also auto on PRs into `main`                          | Lint, typecheck, unit tests, build, a11y ([`ci.yml`](../.github/workflows/ci.yml))                                                                                                                                                                                                                                                                                                                                                | `CI`                                                                               |
| **`/infra-ci`** | Before merging into `dev` when the PR touches migrations, `database.types.ts`, and/or PowerSync sync rules | Against **development** (Cloud): Supabase `db push --dry-run`, PowerSync `validate`, and local Docker types check if migration paths changed ([`supabase-migrations.yml`](../.github/workflows/supabase-migrations.yml), [`powersync-sync-config.yml`](../.github/workflows/powersync-sync-config.yml), [`supabase-db-types-pr.yml`](../.github/workflows/supabase-db-types-pr.yml)). Irrelevant paths are skipped (still green). | `infra / supabase-dry-run`, `infra / powersync-validate`, `infra / database-types` |
| **`/preview`**  | When you want a Vercel preview of user/practitioner web                                                    | Preview deploy ([`vercel-preview.yml`](../.github/workflows/vercel-preview.yml))                                                                                                                                                                                                                                                                                                                                                  | (posts a PR comment with the URL; not a required status)                           |
