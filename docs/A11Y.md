...
Filled primary actions (submit buttons, main CTAs) use **`bg-app-primary-solid`** with **`text-app-on-primary-solid`**, not `bg-app-primary` + `text-white`. In dark mode, `--app-primary` is a lighter link accent (`#60a5fa`); solid buttons keep **`#1d4ed8`** so white label text meets WCAG contrast. Link and chart accents continue to use `--app-primary`.
